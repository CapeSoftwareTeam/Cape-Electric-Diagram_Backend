...
public class NodesFileStorageImpl implements NodesFileStorageService {
	...
	public ResponseFile downloadFile(String nodeId) throws IOException {
		if (nodeId != null && nodeId != "") {
			ResponseFile fileDB = fileUploadRepository.findByNodeId(nodeId).get();
	...
	public void updateFile(MultipartFile file,String component,String mainFileName, Integer fileId, String fileSize) throws SerialException, SQLException, IOException {
		if (fileId != null && fileId != 0) {
			ResponseFile fileDB = fileUploadRepository.findById(fileId).get();
	...
	public ResponseFile retrieveFileNameByNodeId(String nodeId) throws IOException {
		if (nodeId != null && nodeId != "") {
			Optional<ResponseFile> fileDB = fileUploadRepository.findByNodeId(nodeId);
			if(fileDB.isPresent()) {
